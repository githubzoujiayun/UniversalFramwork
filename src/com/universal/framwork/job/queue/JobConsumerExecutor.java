...
import com.universal.framwork.job.JobDelivery;
...
public class JobConsumerExecutor {
    public static final String TAG="JobConsumerExecutor";
    ...
    public JobConsumerExecutor(JobConfig config, Contract contract) {
    ...
    private void addConsumer() {
        LogUtil.d("yzy","adding another consumering");
        synchronized (threadGroup) {
    ...
    private boolean isAboveLoadFactor(boolean inConsumerThread) {
        synchronized (threadGroup) {
            ...
            boolean res =
                consumerCnt==0|| consumerCnt < minConsumerSize ||
                    consumerCnt * loadFactor < contract.countRemainingReadyJobs() + runningJobHolders.size();
                LogUtil.d("yzy",String.format("%s: load factor check. %s = (%d < %d)|| (%d * %d < %d + %d). consumer thread: %s", Thread.currentThread().getName(), res,
    ...
    private String createRunningJobHolderKey(JobHolder jobHolder) {
        return createRunningJobHolderKey(jobHolder.getId());
    }

    private String createRunningJobHolderKey(long id) {
        return id + "_f";
    }
    ...
    public boolean isRunning(long id) {
        return runningJobHolders.containsKey(createRunningJobHolderKey(id));
    }
    ...
    private static class JobConsumer implements Runnable {
        ...
        public void run() {
            ...
            do {
                try {
                        if(didRunOnce == false) {
                            LogUtil.d(TAG,String.format("starting consumer %s", Thread.currentThread().getName()));
                            didRunOnce = true;
                        } else {
                            LogUtil.d(TAG,String.format("re-running consumer %s", Thread.currentThread().getName()));
                        }
                    JobHolder nextJob;
                    do {
                        ...
                        if (nextJob != null) {
                            ...
                            if (nextJob.safeRun(nextJob.getRunCount(),contract.getJobDelivery())) {
                    ...
                    canDie = executor.canIDie();
                        if(canDie) {
                            LogUtil.d("yzy",String.format("finishing consumer %s", Thread.currentThread().getName()));
                        } else {
                            LogUtil.d("yzy",String.format("didn't allow me to die, re-running %s", Thread.currentThread().getName()) );
                        }
                }
            } while (!canDie);
        }
